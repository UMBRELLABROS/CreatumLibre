...
class CreatumLibreApp:
    ...
    def __init__(self, root):
        ...
        self.root.geometry(
            "1200x800+100+50"
        )  # start with a reasonable size and position
    ...
    def create_menu(self, root):
        ...
        file_menu.add_command(
            label=lang.get_text("open_file"), command=self.files_manager.load_picture
        )
        file_menu.add_command(
            label=lang.get_text("save_file"), command=self.files_manager.save_picture
        )
        file_menu.add_separator()
        file_menu.add_command(
            label=lang.get_text("exit_application"), command=root.quit
        )
        menu.add_cascade(label=lang.get_text("file"), menu=file_menu)

        zoom_menu = tk.Menu(menu, tearoff=0)
        zoom_menu.add_command(
            label=lang.get_text("zoom_in"),
            accelerator="Ctrl++",
            command=self.zoom_manager.zoom_in,
        )
        zoom_menu.add_command(
            label=lang.get_text("zoom_out"),
            accelerator="Ctrl+-",
            command=self.zoom_manager.zoom_out,
        )
        zoom_menu.add_command(
            label=lang.get_text("reset_zoom"),
            accelerator="Ctrl+.",
            command=self.zoom_manager.reset_zoom,
        )
        zoom_menu.add_command(
            label=lang.get_text("fit_to_frame"),
            accelerator="Ctrl+#",
            command=self.zoom_manager.fit_to_frame,
        )
